...
var _ = Describe("Get", func() {
	...
	DescribeTable("getting test resources",
		...
		Entry("should get ConfigMap with template string and multiple binding maps", testCase{
			objs: []client.Object{
		...
		Entry("should get multiple custom resources with typed objects", testCase{
			objs: []client.Object{
			...
			validateObjects: func(args []interface{}) {
				objs := args[0].([]client.Object)
				tr1 := objs[0].(*testutil.TestResource)
				tr2 := objs[1].(*testutil.TestResource)
				Expect(tr1.Data).To(Equal("data1"))
		...
		Entry("should get multiple resources with template string and bindings", testCase{
			objs: []client.Object{
		...
		Entry("should get multiple resources with template string and multiple binding maps", testCase{
			objs: []client.Object{
		...
		Entry("should return get error for objects", testCase{
			objs: []client.Object{
				testutil.NewConfigMap("test-cm1", "default", map[string]string{"key1": "value1"}),
				testutil.NewConfigMap("test-cm2", "default", map[string]string{"key2": "value2"}),
			},
		...
		Entry("should return get error for template", testCase{
			objs: []client.Object{
				testutil.NewConfigMap("test-cm", "default", map[string]string{"foo": "bar"}),
			},
			client: &MockClient{
...
var _ = Describe("GetFunc", func() {
	type testCase struct {
		...
		expectedFailureLogs []string
		validateObjects     func([]interface{}) // For validating retrieved object state
		pollTest            bool                // Whether to test polling behavior
	}
	DescribeTable("getting test resources with function",
		func(tc testCase) {
			// Create objects (if not testing polling)
			if !tc.pollTest {
				for _, obj := range tc.objs {
					Expect(tc.client.Create(ctx, obj)).To(Succeed(), "failed to create object")
				}
			...
			if tc.pollTest {
				// Test polling behavior - first call should fail, then create resource, then succeed
				err := getFunc()
				Expect(err).To(HaveOccurred(), "expected first call to fail")
				...
				err = getFunc()
				Expect(err).NotTo(HaveOccurred(), "expected second call to succeed")
			} else {
		...
		Entry("should handle polling with multiple objects", testCase{
			objs: []client.Object{
				testutil.NewConfigMap("test-cm1", "default", map[string]string{"key1": "value1"}),
				testutil.NewConfigMap("test-cm2", "default", map[string]string{"key2": "value2"}),
			},
		...
		Entry("should handle polling with template", testCase{
			objs: []client.Object{
				testutil.NewConfigMap("test-cm", "default", map[string]string{"foo": "bar"}),
			},
			client: &MockClient{Client: testutil.NewStandardFakeClient()},
